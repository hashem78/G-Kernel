	...
	uint16_t res)
{
	...
	if (rc < 0)
		return rc;
#if 1 //JAY dimension change
	if(res == 3)
		res = 0;
	else if (res == 4)
		res = 2;
	CDBG("[JAY] dimension setting: %d\n",res);
#endif
...
{
	int32_t rc = 0;
#if 1 //JAY sensor revision no. check
	uint16_t rev_no = 0;
	rc = msm_camera_i2c_read(
			s_ctrl->sensor_i2c_client,
			s_ctrl->sensor_id_info->sensor_id_reg_addr, &rev_no,
			MSM_CAMERA_I2C_WORD_DATA);
	CDBG("[JAY] device ID: %x\n",rev_no);
	if(rev_no == 0x6b20){ //JF front camera chip ID check
	rc = msm_camera_i2c_read(
			s_ctrl->sensor_i2c_client,
			0x0002, &rev_no,
			MSM_CAMERA_I2C_WORD_DATA);
	CDBG("[JAY] rev NO: %x\n",rev_no);		//rev no check
	rev_no = rev_no >> 8;					//only use MSB
	CDBG("[JAY] SHIFTED_rev_no: %x\n",rev_no);

	if(!rev_no){
		CDBG("[JAY] This is old(before 0.99) ver sensor\n");
		if(res == 0)
			res = 3;
		else if (res == 2)
			res = 4;
...
int32_t msm_sensor_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp)
{
	...
	case CFG_POWER_DOWN:
		if (s_ctrl->func_tbl->sensor_power_down)
			...
			rc = -EFAULT;
		break;
    case CFG_SET_STREAMING_MODE:
		if (s_ctrl->func_tbl->sensor_set_streaming_mode)
			rc = s_ctrl->func_tbl->sensor_set_streaming_mode(
    ...
    case CFG_SET_VISION_AE:
		if (s_ctrl->func_tbl->sensor_set_vision_ae_control)
			rc = s_ctrl->func_tbl->sensor_set_vision_ae_control(
	...
	const struct i2c_device_id *id)
{
	...
	goto power_down;
#if 1 //temp insook0321 #if !defined(CONFIG_JC) && !defined(CONFIG_S5K6B2YX)
probe_fail:
	...
	if (on) {
		...
		if (rc < 0) {
			...
			if (s_ctrl->func_tbl->sensor_match_id)
				...
				rc = 0;/*msm_sensor_match_id(s_ctrl);*/
			if (rc < 0) {
